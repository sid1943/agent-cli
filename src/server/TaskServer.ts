...
export class TaskServer {
  ...
  async initialize(): Promise<void> {
    // Load existing state
    const existingState = await this.stateManager.readState<ServerState>();
   ...
   */
  async registerAgent(agentId: string, info: Partial<AgentInfo>): Promise<AgentInfo> {
   ...
   */
  async updateHeartbeat(agentId: string, status?: AgentStatus, progress?: number): Promise<void> {
    const agent = this.state.agents[agentId];
    if (!agent) return;
  ...
  async assignTask(taskId: string, agentId: string): Promise<Task | null> {
    ...
    // Send task to agent
    await this.messageQueue.sendToAgent(agentId, createMessage(
      'TASK_ASSIGN',
      'server',
      { task },
      agentId
  ...
  async startTask(taskId: string, agentId: string): Promise<void> {
    const task = this.state.tasks[taskId];
    if (!task || task.assignedAgent !== agentId) return;
   ...
   * Complete task
   */
  async completeTask(taskId: string, agentId: string, result: TaskResult): Promise<void> {
    const task = this.state.tasks[taskId];
    const agent = this.state.agents[agentId];
  ...
  private emitEvent<T>(type: EventType, source: string, data: T): void {
    const event: Event<T> = {
      type,
      timestamp: Date.now(),
      source,
  ...
  private cleanupDisconnectedAgents(): void {
    const now = Date.now();
