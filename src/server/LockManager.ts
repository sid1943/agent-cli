...
export class LockManager {
  ...
  constructor(projectPath: string, defaultTimeout: number = 300000) {
  ...
  acquireLocks(request: LockRequest): LockResult {
    ...
    const failed: string[] = [];
    const conflicts: { path: string; heldBy: string }[] = [];
   ...
   */
  checkConflicts(agentId: string, paths: string[]): { path: string; heldBy: string }[] {
    this.cleanExpiredLocks();
    const conflicts: { path: string; heldBy: string }[] = [];
  ...
  private isCompatibleLock(existing: FileLock['lockType'], requested: FileLock['lockType']): boolean {
  ...
  // Clean expired locks
  private cleanExpiredLocks(): number {
    const now = Date.now();
    let cleaned = 0;
  ...
  private loadLocks(): void {
    ...
    try {
      if (fs.existsSync(locksFile)) {
        ...
        const data = JSON.parse(content) as FileLock[];
