...
import { findProjectRoot, getCoordinatorPaths, loadConfig } from '../shared/config.js';
import { StateManager } from '../shared/protocol.js';
import { ServerState, AgentInfo, Task } from '../shared/types.js';
...
function printAgents(agents: AgentInfo[]): void {
  printHeader('AGENTS');
...
function printTasks(tasks: Task[], queue: string[]): void {
  printHeader('TASKS');
...
function printLocks(locks: any[]): void {
...
async function main() {
  ...
  const state = await stateManager.readState<ServerState>();
